...
class StatBlock:
    ...
    def __add__(self, other: 'StatBlock') -> 'StatBlock':
        return StatBlock(
            self.hp + other.hp, self.atk + other.atk, self.spd + other.spd,
            self.defense + other.defense, self.res + other.res
        )
...
class Unit:
    ...
    """
    def __init__(self, name: str, movement_type: MovementType, weapon_type: WeaponType, color: Color,
                 hp: int, atk: int, spd: int, defense: int, res: int, 
                 dragonflower: int = 0, merges: int = 0,
                 superboon: list[str] | None = None, superbane: list[str] | None = None, 
                 boon: str | None = None, bane: str | None = None, 
                 floret: str | None = None, 
                 weapon: Skill | None = None, special: Skill | None = None, 
                 a_slot: Skill | None = None, b_slot: Skill | None = None, 
                 c_slot: Skill | None = None, s_slot: Skill | None = None, 
                 x_slot: Skill | None = None):
        
        self.name, self.movement_type, self.weapon_type, self.color = name, movement_type, weapon_type, color
    ...
    def _initialize_stats(self):
        ...
        """
        d = {"hp": self.base_stats.hp, "atk": self.base_stats.atk, "spd": self.base_stats.spd, 
             "defense": self.base_stats.defense, "res": self.base_stats.res}
        
        if self.bane: d[self.bane] -= (4 if self.bane in self.superbane else 3)
        if self.boon: d[self.boon] += (4 if self.boon in self.superboon else 3)
        if self.floret and self.floret != self.boon: d[self.floret] += (4 if self.floret in self.superboon else 3)
    ...
    def equipped_items(self) -> list[Skill]:
        ...
        return [s for s in [self.weapon, self.special, self.a_slot, self.b_slot, self.c_slot, self.s_slot, self.x_slot] if s]

    def get_visible_stat(self, name: str) -> int:
        """Calculates the 'stat-screen' value including buffs, debuffs, and visible skill stats."""
        val = getattr(self.base_stats, name) + getattr(self.visible_buffs, name) - getattr(self.visible_debuffs, name)
    ...
    def has_keyword(self, keyword: str) -> bool:
        ...
        if any(keyword in item.utilities.keywords for item in self.equipped_items):
            return True
        return any(keyword in STATUS_EFFECT_DATABASE[s]["utilities"].keywords 
                   for s in self.active_statuses if s in STATUS_EFFECT_DATABASE)
